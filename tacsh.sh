...
funcGenTacSh() {
    ...
    funcAddTacSh "rlsh () {"
    funcAddTacSh "\tif [ -f \"\$HOME/.$shPf\" ] && [ -f \"\$HOME/.$shRc\" ]; then source \"\$HOME/.$shPf\"; source \"\$HOME/.$shRc\"; RLSH_MSG=\"$shPf and $shRc\""
    funcAddTacSh "\telif [ -f \"\$HOME/.$shPf\" ]; then source \"\$HOME/.$shPf\"; RLSH_MSG=\"$shPf\""
    funcAddTacSh "\telif [ -f \"\$HOME/.$shRc\" ]; then source \"\$HOME/.$shRc\"; RLSH_MSG=\"$shRc\""
    funcAddTacSh "\telse echo \"not found .$shPf and .$shRc\"; exit 1; fi"
    funcAddTacSh "\techo \"reloaded \$RLSH_MSG\""
    funcAddTacSh "}"
    ...
    funcAddTacSh "\t\t\tif [[ \$1 == 0 ]]; then pwd"
    funcAddTacSh "\t\t\telse printf -v cdpFull '%*s' \$1; cd \"\${cdpFull// /\"../\"}\"; fi"
    if [ $keName = "Darwin" ]; then 
