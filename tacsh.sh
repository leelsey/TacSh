...
funcGenTacSh() {
    ...
    funcAddTacSh "TACSH_HOME=\"$tacshDirPath\""
    if [ $keName = "Darwin" ]; then funcAddTacSh "ICLOUD_HOME=$iCloudPath"; fi
    funcAddTacSh "DROPBOX_HOME=$dropboxPath"
    funcAddTacSh "tacsh () {"
    funcAddTacSh "\tif [[ \$1 == ver ]] || [[ \$1 == version ]]; then echo \"ver \$TACSH_VERSION\""
    if [ $keName = "Darwin" ] || [ $keName = "MinGW64" ]; then
        funcAddTacSh "\telif [[ \$1 == ls ]] || [[ \$1 == list ]]; then cat \"\$TACSH_HOME/tac.sh\""
        funcAddTacSh "\telif [[ \$1 == conf ]] || [[ \$1 == config ]] || [[ \$1 == configure ]]; then /usr/bin/vi \"\$TACSH_HOME/tac.sh\""
    elif [ $keName = "Linux" ]; then
        funcAddTacSh "\telif [[ \$1 == ls ]] || [[ \$1 == list ]]; then cat \"\$TACSH_HOME/tac.$shName\""
        funcAddTacSh "\telif [[ \$1 == conf ]] || [[ \$1 == config ]] || [[ \$1 == configure ]]; then /usr/bin/vi \"\$TACSH_HOME/tac.$shName\""
    fi
    ...
    funcAddTacSh "rlsh () {"
    funcAddTacSh "\tif [ ! -f \"\$HOME/.$shEnv\" ] && [ ! -f \"\$HOME/.$shPf\" ] && [ ! -f \"\$HOME/.$shRc\" ] && [ ! -f \"\$HOME/.$shLin\" ] && [ ! -f \"\$HOME/.$shLout\" ]"
    funcAddTacSh "\tthen echo \"not found $shType environment files\"; exit 1; fi"
	funcAddTacSh "\tif [ -f \"\$HOME/.$shEnv\" ]; then source \"\$HOME/.$shEnv\"; RLSH_MSG=\" $shEnv\"; fi"
	funcAddTacSh "\tif [ -f \"\$HOME/.$shPf\" ]; then source \"\$HOME/.$shPf\"; RLSH_MSG=\$RLSH_MSG\" $shPf\"; fi"
	funcAddTacSh "\tif [ -f \"\$HOME/.$shRc\" ]; then source \"\$HOME/.$shRc\"; RLSH_MSG=\$RLSH_MSG\" $shRc\"; fi"
	funcAddTacSh "\tif [ -f \"\$HOME/.$shLin\" ]; then source \"\$HOME/.$shLin\"; RLSH_MSG=\$RLSH_MSG\" $shLin\"; fi"
	funcAddTacSh "\tif [ -f \"\$HOME/.$shLout\" ]; then source \"\$HOME/.$shLout\"; RLSH_MSG=\$RLSH_MSG\" $shLout\"; fi"
	funcAddTacSh "\techo \"reloaded $shType environment files:\$RLSH_MSG\""
    funcAddTacSh "}"
    funcAddTacSh "vienv () { /usr/bin/vi \"\$HOME/.$shEnv\"; }"
    funcAddTacSh "viprofile () { /usr/bin/vi \"\$HOME/.$shPf\"; }"
    funcAddTacSh "virc () { /usr/bin/vi \"\$HOME/.$shRc\"; }"
    funcAddTacSh "vilogin () { /usr/bin/vi \"\$HOME/.$shLin\"; }"
    funcAddTacSh "vilogout () { /usr/bin/vi \"\$HOME/.$shLout\"; }"
    ...
    if [ $keName = "Darwin" ]; then
        ...
        funcAddTacSh "\t\tif [[ \$1 =~ on ]]; then sudo pmset -c disablesleep 0"
        funcAddTacSh "\t\telif [[ \$1 =~ off ]]; then sudo pmset -c disablesleep 1"
    ...
    funcAddTacSh "shspeed () { if [ -z \$1 ]; then 1=1; fi; for i in {1..\$1}; do /usr/bin/time \$SHELL -i -c exit; done; }"
    funcAddTacSh "p () {"
    funcAddTacSh "\tif [ \$# -eq 0 ]; then cd .."
    funcAddTacSh "\telif [ \$# -eq 1 ]; then"
    funcAddTacSh "\t\tif [[ \$1 =~ '^[0-9]+$' ]]; then if [[ \$1 == 0 ]]; then pwd; else printf -v cdpFull '%*s' \$1; cd \"\${cdpFull// /\"../\"}\"; fi"
    if [ $keName = "Darwin" ]; then 
        funcAddTacSh "\t\telif [[ \$1 =~ '^[y]+$' ]]; then pwd; pwd | pbcopy"
    fi
    funcAddTacSh "\t\telif [[ \$1 =~ '^[p]+$' ]]; then pwd"
    funcAddTacSh "\t\telif [[ \$1 =~ '^[b]+$' ]] || [[ \$1 == - ]]; then cd -"
    funcAddTacSh "\t\telif [[ \$1 =~ '^[r]+$' ]] || [[ \$1 == / ]]; then cd /"
    funcAddTacSh "\t\telif [[ \$1 =~ '^[h]+$' ]] || [[ \$1 == ~ ]]; then cd ~"
    if [ $keName = "Darwin" ]; then
        funcAddTacSh "\t\telif [[ \$1 == ap ]] || [[ \$1 == app ]] || [[ \$1 == App ]]; then cd /Applications"
    fi
    funcAddTacSh "\t\telif [[ \$1 == ds ]] || [[ \$1 == des ]] || [[ \$1 == Des ]]; then cd ~/Desktop"
    funcAddTacSh "\t\telif [[ \$1 == dc ]] || [[ \$1 == doc ]] || [[ \$1 == Doc ]]; then cd ~/Documents"
    funcAddTacSh "\t\telif [[ \$1 == dw ]] || [[ \$1 == dow ]] || [[ \$1 == Dow ]]; then cd ~/Downloads"
    if [ $keName = "Darwin" ]; then
        funcAddTacSh "\t\telif [[ \$1 == lb ]] || [[ \$1 == lib ]] || [[ \$1 == Lib ]]; then cd ~/Library"
        funcAddTacSh "\t\telif [[ \$1 == mv ]] || [[ \$1 == mov ]] || [[ \$1 == Mov ]]; then cd ~/Movies"
    fi
    ...
    funcAddTacSh "\t\telif [[ \$1 == db ]] || [[ \$1 == drb ]] || [[ \$1 == Drb ]]; then"
    funcAddTacSh "\t\t\tif [ -d $dropboxPath ]; then cd $dropboxPath; else echo \"p: wrong usage, try p -h\"; fi"
    ...
    if [ $keName = "Darwin" ]; then 
        funcAddTacSh "\t\t\techo \"p y: output current directory with copy\""
    fi
    ...
    if [ $keName = "Darwin" ]; then
        funcAddTacSh "\t\t\techo \"p lb, lib: change direcotry to library directory\""
    ...
    funcAddTacSh "\t\t\techo \"p db, drb: change direcotry to dropbox directory\""
    funcAddTacSh "\t\t\techo \"p -h: output usage\""
    funcAddTacSh "\t\telse echo \"p: wrong usage, try p -h\"; fi"
    funcAddTacSh "\telse echo \"p: wrong usage, try p -h\"; fi"
    funcAddTacSh "}"
    funcAddTacSh "jctl () {"
    funcAddTacSh "\tif [ \"\$#\" -eq 0 ]; then /usr/libexec/java_home -V"
    funcAddTacSh "\telif [ \"\$#\" -eq 1 ]; then unset JAVA_HOME; export JAVA_HOME=\$(/usr/libexec/java_home -v \"\$1\"); java -version"
    funcAddTacSh "\telse echo \"javahome: wrong usage\"; fi"
    funcAddTacSh "}"
    funcAddTacSh "dockerun () {"
    funcAddTacSh "\tif ! docker info > /dev/null 2>&1; then echo \"dockerun false: Docker isn't running\""
    funcAddTacSh "\telse echo \"dockerun true: Docker is running\"; fi"
    funcAddTacSh "}"
    if [ $keName = "Darwin" ]; then
        funcAddTacSh "chicn () {"
        funcAddTacSh "\tif [ \$# -eq 2 ]; then"
        funcAddTacSh "\t\tif [[ \"\$1\" =~ ^https?:// ]]; then curl -sLo /tmp/ic \"\$1\"; 1=/tmp/iconchange; fi"
        funcAddTacSh "\t\tif ! [ -f \$1 ]; then echo \"chicn: cannot stat '\$1': No such file\""
        funcAddTacSh "\t\telif ! [ -d \$2 ]; then echo \"chicn: cannot stat '\$2': No such directory\""
        funcAddTacSh "\t\telse"
        ...
        funcAddTacSh "\telse echo \"chicn: wrong usage\"; fi"
        funcAddTacSh "}"
    fi
