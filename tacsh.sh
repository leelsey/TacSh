...
funcGenTacSh() {
    ...
    if [ $keName = "Darwin" ] || [ $keName = "MinGW64" ]; then
        ...
        funcAddTacSh "\telif [[ \$1 == conf ]] || [[ \$1 == config ]] || [[ \$1 == configure ]]; then vi \"\$TACSH_HOME/tac.sh\""
    elif [ $keName = "Linux" ]; then
        funcAddTacSh "\telif [[ \$1 == ls ]] || [[ \$1 == list ]]; then cat \"\$TACSH_HOME/tac.$shName\""
        funcAddTacSh "\telif [[ \$1 == conf ]] || [[ \$1 == config ]] || [[ \$1 == configure ]]; then vi \"\$TACSH_HOME/tac.$shName\""
    fi
    ...
    funcAddTacSh "rlsh () {"
    funcAddTacSh "\tif [ -f \"\$HOME/.$shPf\" ] || [ -f \"\$HOME/.$shRc\" ]; then source \"\$HOME/.$shPf\" && source \"\$HOME/.$shRc\" ; echo \"reloaded .$shPf and .$shRc\""
    funcAddTacSh "\telif [ -f \"\$HOME/.$shRc\" ]; then source \"\$HOME/.$shRc\" ; echo \"reloaded .$shRc\""
    funcAddTacSh "\telif [ -f \"\$HOME/.$shPf\" ]; then source \"\$HOME/.$shPf\" ; echo \"reloaded .$shPf\""
    funcAddTacSh "\telse echo \"shrl: No environment file found\" ; fi"
    funcAddTacSh "}"
