...
funMain() {
    ...
    if [ "$(uname)" = "Darwin" ]; then
        addFile "      echo \"p ap, app: move to applications directory\""
    fi
    addFile "      echo \"p ds, des: move to desktop directory\""
    addFile "      echo \"p dc, doc: move to documents directory\""
    addFile "      echo \"p dw, dow: move to downloads directory\""
    if [ "$(uname)" = "Darwin" ]; then
        addFile "      echo \"p lb, lib: move to movies directory\""
        addFile "      echo \"p mv, mov: move to movies directory\""
    fi
    addFile "      echo \"p ms, mus: move to music directory\""
    addFile "      echo \"p pc, pic: move to pictures directory\""
    if [ "$(uname)" = "Darwin" ]; then
        addFile "      echo \"p ic, cl, icl, clo: move to icloud directory\""    
    elif [ "$(uname)" = "Linux" ]; then
        addFile "      echo \"p vd, vid: move to videos directory\""
    fi
    addFile "      echo \"p dr, dro, drp: move to dropbox directory\""
